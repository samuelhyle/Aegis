    ...
    public boolean test(Person person, long time) {
      ...
      switch (unit) {
        ...
        case "months":
          ...
          break;
        case "weeks":
          age = person.ageInDecimalYears(time) * 365.2425 / 7.0;
          break;
        case "days":
          age = person.ageInDecimalYears(time) * 365.2425;
          break;
        case "hours":
          age = person.ageInDecimalYears(time) * 365.2425 * 24.0;
          break;
        default:
  ...
  public static class Observation extends Logic {
    ...
    public boolean test(Person person, long time) {
      ...
      if (this.codes != null) {
        for (Code code : this.codes) {
          ...
          if (last == null && person.hasMultipleRecords) {
            ...
            // then look in all split records.
            for (HealthRecord record : person.records.values()) {
              last = record.getLatestObservation(code.code);
              if (last != null) {
                break;
              }
            }
            if (last == null) {
              // If still not found, look in the module history.
      ...
      if (operator.equals("is nil")) {
        ...
        if (this.codes != null) {
          // If using split records and observation is still null after searching all records,
