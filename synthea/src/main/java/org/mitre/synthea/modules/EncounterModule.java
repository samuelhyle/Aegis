...
public final class EncounterModule extends Module {
  ...
  public long recommendedTimeBetweenWellnessVisits(Person person, long time) {
    ...
    // Check condition-specific visit frequencies
    long conditionInterval = Long.MAX_VALUE;

    // Diabetes: HbA1c every 3 months
    if (person.record.conditionActive("Diabetes")
        || person.record.conditionActive("Diabetes type 2")) {
      conditionInterval = Math.min(conditionInterval,
          Utilities.convertTime("months", 3));
    }

    // Hypertension: monitoring every 3 months
    if (person.record.conditionActive("Hypertension")) {
      conditionInterval = Math.min(conditionInterval,
          Utilities.convertTime("months", 3));
    }

    // Chronic kidney disease: monitoring every 6 months
    if (person.record.conditionActive("Chronic kidney disease stage 1")
        || person.record.conditionActive("Chronic kidney disease stage 2")
        || person.record.conditionActive("Chronic kidney disease stage 3")) {
      conditionInterval = Math.min(conditionInterval,
          Utilities.convertTime("months", 6));
    }

    // Contraception: annual visit
    if (person.record.careplanActive("Contraception")) {
      conditionInterval = Math.min(conditionInterval,
          Utilities.convertTime("years", 1));
    }

    // Use the most frequent interval among age-based and condition-based
    if (conditionInterval < Long.MAX_VALUE) {
      interval = Math.min(interval, conditionInterval);
    }
