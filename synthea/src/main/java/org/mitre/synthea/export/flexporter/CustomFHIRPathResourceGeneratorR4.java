...
public class CustomFHIRPathResourceGeneratorR4<T extends Resource> {
  ...
  private void handleWhereFunctionParam(ExpressionNode param) {
        ...
        for (IBase operationValue : operationValues) {
          ...
          switch (param.getOperation()) {
            ...
            case NotEquals:
              ...
              break;
            case Greater:
              if (compareValues(primitive.getValueAsString(), matchingValue) > 0) {
                matchingNodes.add(node);
              }
              break;
            case GreaterOrEqual:
            ...
            case LessOrEqual:
              ...
              break;
            case Contains:
              if (primitive.getValueAsString().contains(matchingValue)) {
                matchingNodes.add(node);
              }
              break;
            case In:
              if (matchingValue.contains(primitive.getValueAsString())) {
                matchingNodes.add(node);
              }
              break;
            case And:
            ...
            case Xor:
            default:
              // Other operations: add node as matching
              matchingNodes.add(node);
          }
        }
      }
    ...
    if (matchingNodes.size() == 0) {
      ...
      switch (param.getOperation()) {
        ...
        case LessThan:
        case LessOrEqual:
          // For comparison operators on unlabeled nodes, add them to matching
          // since we can't compare without a value
          matchingNodes.addAll(unlabeledNodes);
          break;
        case Contains:
        case In:
          // For string operations on unlabeled nodes, add them to matching
          matchingNodes.addAll(unlabeledNodes);
          break;
        case And:
        case As:
        ...
        case Xor:
        default:
          // Other operations: add unlabeled nodes as matching
          matchingNodes.addAll(unlabeledNodes);
      }
    }
  ...
  private int compareValues(String value1, String value2) {
    try {
      java.math.BigDecimal num1 = new java.math.BigDecimal(value1);
      java.math.BigDecimal num2 = new java.math.BigDecimal(value2);
