...
public class BeneficiaryExporter extends RIFExporter {
  ...
  public String export(Person person, long startTime, long stopTime) throws IOException {
    ...
    for (int year = endYear - yearsOfHistory; year <= endYear; year++) {
      ...
      // Subtract months before coverage started (e.g., before 65 or before disability)
      // If coverage started mid-year, adjust the month count accordingly
      if (coverageStartDate > startOfYearTimeStamp) {
        int coverageStartMonth = Utilities.getMonth(coverageStartDate);
        int monthsBeforeCoverage = coverageStartMonth - 1; // Months before coverage started
        monthCount = Math.max(0, monthCount - monthsBeforeCoverage);
      }
      ...
      // RTI Race Code: The RTI algorithm uses first name, surname, and geographic location
      // to impute race. Full implementation requires name probability tables and county-level
      // demographic data. For now, use the same race code as BENE_RACE_CD.
    ...
    if (medicareStatusCode.equals("00")) {
      ...
      if (dualEligibleStatusCode.equals("NA") || dualEligibleStatusCode.equals("09")) {
        ...
        if (partBEnrollee) {
          ...
          return "2"; // Part B state buy-in only
        }
      }
    }
  }
