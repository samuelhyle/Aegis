...
public class FhirDstu2 {
  ...
  private static Entry medicationClaim(Entry personEntry,
    ...
    // pharmacy claim for medication
    claimResource.setType(ClaimTypeEnum.PHARMACY);
    ...
    // add medication as a claim item with cost
    ca.uhn.fhir.model.dstu2.resource.Claim.Item medicationItem =
        new ca.uhn.fhir.model.dstu2.resource.Claim.Item();
    medicationItem.setSequence(new PositiveIntDt(1));

    // item type is pharmacy
    CodingDt itemType = new CodingDt();
    itemType.setSystem("http://hl7.org/fhir/v3/ActCode")
        .setCode("PHARMACY")
        .setDisplay("pharmacy");
    medicationItem.setType(itemType);

    // service should match the medication code from the claim's main entry
    HealthRecord.Entry claimEntry = claim.mainEntry.entry;
    if (claimEntry.codes != null && !claimEntry.codes.isEmpty()) {
      Code code = claimEntry.codes.get(0);
      ...
      medicationItem.setService(itemService);
    }
    ...
    medicationItem.setNet(moneyResource);
  ...
  private static Entry allergy(RandomNumberGenerator rand, Entry personEntry, Bundle bundle,
    ...
    if (allergy instanceof HealthRecord.Allergy
        && ((HealthRecord.Allergy) allergy).category != null) {
      switch (((HealthRecord.Allergy) allergy).category) {
        case "medication":
        ...
        case "biologic":
        case "other":
          category = AllergyIntoleranceCategoryEnum.OTHER;
          break;
        default:
          category = AllergyIntoleranceCategoryEnum.FOOD;
      }
    }
    allergyResource.setCategory(category);
